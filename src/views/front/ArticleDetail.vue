    ...
    <div class="AsideNav">
      ...
      <div class="userInfor">
        ...
        <div class="avaAndNickname">
          <div class="ava">
            <el-image :src="author.avatarUrl"
                      style="width: 70px; height: 70px; border-radius: 50%;flex: 1; "
                      @click="$router.push('/front/personalCenter/newArticleOfPersonal?id='+author.id)"></el-image>
          </div>
      ...
      <div class="articleOfUser">
        ...
        <div style="flex: 16;">
          <div class="theTitleOfArticle2"
               v-for="item in tableData"
               :key="item.id"
               style="margin-top: 2px;padding-top: 10px">
            <div class="article-head"
                 style=" cursor: pointer;margin:0px 5px ;line-height: 22px;height: 22px;text-overflow: ellipsis;
overflow: hidden;color: #000; font-weight: normal"
                 @click="pushPage(item.id)">{{item.name}}</div>
    ...
    <div class="articleDetail">
      ...
      <div class="headOfArticle">
        <div class="head">
          ...
          <div style="flex: 1">
            <el-button v-if="!showSign"
                       type="primary"
                       round
                       @click="collectArticle">收藏文章</el-button>
            <el-button v-if="showSign"
                       type="primary"
                       round
                       @click="deleteCollection">取消收藏</el-button>
          </div>
        </div>
        <!--            文章所属专题或者标签-->
        <div class="tip">
      ...
      <div class="bodyOfArticle">
        <mavon-editor style="background: #FFFFFF"
                      class="md"
      ...
      <div class="replyOfArticle">
        ...
        <div class="releaseComment">
          <el-image :src="user.avatarUrl"
                    style="width: 70px; height: 70px; border-radius: 50%;margin-top: 30px;flex: 1"></el-image>
            ...
            <div style="flex: 1;font-size: 12px;color: #99A2AA;display: flex">
              ...
              <div style="flex: 5">
                <el-link type="primary"
                         @click="handleReply(item.id)">回复</el-link>
                <el-link type="danger"
                         @click="del(item.id)"
                         v-if="user.id === item.userId"
                         style="margin-left: 20px">删除</el-link>
              </div>
            </div>
            <div class="childrenRpelyList"
                 v-if="item.children.length">
              <!--                   子集评论列表-->
              <div class="childrenOfReply"
                ...
                <div style="flex: 1;display: flex">
                  <el-image :src="subItem.avatarUrl"
                            style="width: 30px; height: 30px; border-radius: 50%;flex: 1"></el-image>
                </div>
                ...
                <div style="flex: 28;display: flex;flex-direction: column">
                  <div style="display: flex;">
                    ...
                    <!--                                    被回复人的名称-->
                    <div style="flex: 2;font-size: 12px;color: #409EFF;padding: 5px 10px"
                         v-if="subItem.pnickname">
                  ...
                  <div style="flex: 1;display: flex;align-items: center;justify-items: center;margin-top: 10px">
                    ...
                    <div style="flex: 5;display: flex;padding-left: 5px">
                      <el-link type="primary"
                               ...
                               @click="handleReply(subItem.id)">回复</el-link>
                      <el-link type="danger"
                               @click="del(subItem.id)"
                               v-if="user.id === subItem.userId"
                               style="font-size: 12px;margin-left: 20px">删除</el-link>
                    </div>
                  </div>
                </div>
